...
public class BinaryDecisionTreeClassifier implements Classifier {
	private BinaryDecisionTreeNode dtc;
	private DataSet dataset;
	private int depth = -1;
	private boolean useGini = false;

	public BinaryDecisionTreeClassifier() {
	}
	 ...
	 */
	private BinaryDecisionTreeNode trainRecursive(BinaryDecisionTreeNode dt, ArrayList<Example> data,
		...
		if (depth != -1 && (dataset.getAllFeatureIndices().size() - remainingFeatures.size() == depth)) {
			return new BinaryDecisionTreeNode(getMajorityLabel(data));
		} else if (data.size() == 0) {
			return new BinaryDecisionTreeNode(getMajorityLabel(parentData));
		} else if (isSameLabel(data)) {
			return new BinaryDecisionTreeNode(data.get(0).getLabel());
		...
		dt = new BinaryDecisionTreeNode(splitFeature);
	 ...
	 */
	private double classifyRecursive(Example example, BinaryDecisionTreeNode root) {
		if (root.isLeaf()) {
	...
	public static void main(String[] args) {
		final String pathToDataset = "../final-project-starter/data/simple1.csv";
//		final String pathToDataset = "src/data/titanic-train.csv";
		DataSet dataset = new DataSet(pathToDataset, DataSet.CSVFILE);

		BinaryDecisionTreeClassifier dt = new BinaryDecisionTreeClassifier();
		dt.setDepthLimit(-1);
		dt.setGini(false);
