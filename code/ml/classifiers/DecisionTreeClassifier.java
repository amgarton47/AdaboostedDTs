 ...
 * 
 * The classifier handles non-binary features, however, when deciding on splits
 * it considers splitting by comparing zero vs. non-zero for each feature.
 * 
 * @author dkauchak
 *
 */
public class DecisionTreeClassifier implements Classifier {
	...
	private DecisionTreeNode buildTree(ArrayList<Example> currentData, HashSet<Integer> usedFeatures, int depthLimit) {
		...
		if (majority.majorityCount == currentData.size() || usedFeatures.size() == featureIndices.size()
				|| depthLimit == 0) {
	 ...
	 * @param usedFeatures which features have been used already and are NOT
	 *                     eligible for splitting on
	...
	private int getBestFeatureIndex(ArrayList<Example> currentData, HashSet<Integer> usedFeatures) {
		...
		for (int featureIndex : featureIndices) {
			if (!usedFeatures.contains(featureIndex)) {
				...
				if (error < bestFeatureScore || (error == bestFeatureScore && featureIndex < bestFeature)) {
	 ...
	 * @return the split of the data. Entry 0 is the left branch data and entry 1
	 *         the right branch data.
	 */
	...
	/**
	 * given the data, calculate the majority label and how many times it occurs in
	 * the data
	 * 
	 * @param data
